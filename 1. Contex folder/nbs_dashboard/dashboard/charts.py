...
def ca_waterfall(comp_df):
    """Waterfall chart: latest year CA decomposition."""
    ...
    fig = go.Figure(go.Waterfall(
        ...
        y=values + [latest["Current Account"]],
        ...
        text=[f"{v:,.0f}" for v in values] + [f"{latest['Current Account']:,.0f}"],
        textfont=dict(size=11, color=COLORS["muted"],
                      family="Inter, sans-serif"),
    ))

    layout = _base_layout(f"CA Waterfall Decomposition ({year})",
...
def fdi_ca_coverage_chart(cov_df):
    """FDI coverage of CA deficit with rolling average."""
    if cov_df.empty:
        return go.Figure()
